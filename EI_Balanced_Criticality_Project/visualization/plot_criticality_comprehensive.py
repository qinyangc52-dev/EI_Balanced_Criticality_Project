...
def extract_sizes_durations(processed):
    """从processed数据中提取sizes和durations"""
    sizes = None
    durations = None
    
    # 数据在 preprocessing 子字典中
    if 'preprocessing' in processed:
        prep = processed['preprocessing']
        if 'avalanche_sizes' in prep:
            sizes = np.array(prep['avalanche_sizes'])
        if 'avalanche_durations' in prep:
            durations = np.array(prep['avalanche_durations'])
    
    return sizes, durations


def extract_distribution(processed):
    """提取已计算好的概率分布"""
    size_values = None
    size_probs = None
    dur_values = None
    ...
    if 'distribution' in processed:
        dist = processed['distribution']
        if 'size_values' in dist:
            size_values = np.array(dist['size_values'])
            size_probs = np.array(dist['size_probs'])
        if 'duration_values' in dist:
            dur_values = np.array(dist['duration_values'])
            dur_probs = np.array(dist['duration_probs'])
    
    return size_values, size_probs, dur_values, dur_probs
...
def plot_size_distribution(data, ax, tau_critical):
    ...
    # 优先使用已计算好的分布
    size_values, size_probs, _, _ = extract_distribution(processed)
    ...
    # 绘制散点
    ax.scatter(size_values, size_probs, s=20, alpha=0.6, 
               c=COLORS['size'], label='Empirical', edgecolors='none')
    
    # 绘制幂律拟合
    size_exp = processed.get('size_exponent')
    size_xmin = processed.get('size_xmin', 1)
    
    if size_exp and size_xmin:
        # 拟合线范围
        x_fit = np.logspace(np.log10(max(1, size_xmin)), 
                           np.log10(size_values.max()), 50)
        # 归一化常数
        mask_fit = size_values >= size_xmin
        if np.any(mask_fit):
            C = size_probs[mask_fit][0] * (size_values[mask_fit][0] ** size_exp)
...
def plot_duration_distribution(data, ax, tau_critical):
    ...
    # 优先使用已计算好的分布
    _, _, dur_values, dur_probs = extract_distribution(processed)
    
    if dur_values is None:
        # 回退：从原始数据计算
    ...
    # 绘制散点
    ax.scatter(dur_values, dur_probs, s=20, alpha=0.6,
               c=COLORS['duration'], label='Empirical', edgecolors='none')
    
    # 绘制幂律拟合
    dur_exp = processed.get('duration_exponent')
    dur_xmin = processed.get('duration_xmin', 1)
    
    if dur_exp and dur_xmin:
        x_fit = np.logspace(np.log10(max(1, dur_xmin)), 
                           np.log10(dur_values.max()), 50)
        mask_fit = dur_values >= dur_xmin
        if np.any(mask_fit):
            C = dur_probs[mask_fit][0] * (dur_values[mask_fit][0] ** dur_exp)
...
def create_comprehensive_figure():
    ...
    print(f"  Size exponent:     {proc.get('size_exponent', 'N/A'):.3f} (理论: ~2.26)")
    print(f"  Duration exponent: {proc.get('duration_exponent', 'N/A'):.3f} (理论: ~2.54)")
    print(f"  Size KS:           {proc.get('size_ks', 'N/A'):.4f} (正常)")
    print(f"  Duration KS:       {proc.get('duration_ks', 'N/A'):.4f} (正常)")
    print("="*70 + "\n")
